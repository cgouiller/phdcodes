...
tracks(:,8)=[1:length(pos(:,1))];

% tracks(:,5) dit si les part sont dispo ou non, on demarre avec tout le
% monde a 1;
% l'id?e est juste d'affecter le num?ro de la trajectoire dans tracks(:,4);

% numeros de trajectoire actif
ind_actif=find(tracks(:,3)==min(tracks(:,3)));
%
...
for kk=2:max(tracks(:,3))
    ...
    for ll=1:length(ind_actif)
        ...
        if dmin<maxdist
            ...
            elseif dispo==0
                ...
                % car elle peut aller ? deux part differentes
    ...
    % definit les particules qui vont etre track?es
    ...
    ind_actif   = tracks_temp(ind,8);
    
    % les non trouv?es se voient affecter un numero de traj
    
    ind=find(tracks(min_new:max_new,3)==numframe&tracks(min_new:max_new,5)==1);
    tracks_temp = tracks(min_new:max_new,:);
    ind_new_part  = tracks_temp(ind,8);
    ...
    if round(kk/5000)*5000==kk
        disp([kk max(tracks(:,3))]);
    end
