...
% 
% marangoni=1; %1 si les nageurs libèrent du camphre source d'écoulement Marangoni, 0 si non
% asrc=1; %S'il y a du Marangoni, amplitude du flux de camphre libéré
% 
...
nt=11000;%10000;%Nombre de pas de temps à effectuer
...
for in=2:nt
    ...
    if in>1000 && mod(in,47)==0 && compteur <=200 %On enlève le début de la simu et on sauvegarde 200 champs de vitesse assez espacés
...
%% Calcul du spectre du champ de vitesse
    ...
    for j=1:200 %vf contient 200 champs de vitesse répartis sur la simu dont on va moyenner les spectres
        vfx=reshape(mvfx(j,:,:),[128 128]); %On en garde qu'un à chaque fois (le numéro j)
    ...
    Spy=Spaddy/(128*200);
    Sp=(Spx+Spy)/2;
    [aaa,f]=pwelch(vfy(k,:),hanning(128),round(128/2),128,1/(2*pi)); % juste pour récupérer le vecteur fréquence
   
    
    
    %% On sauvegarde tout
    save(strcat('E:\Clément\Mixing\SimuNum\Resultats\N',num2str(npart),'ampec',num2str(amp_ec)),'ma','mx','my','muxp','muyp','mvsx','mvsy','npart','Dnag','inertie','taup','marangoni','asrc','advection','ecoulement','param_ecexterne','amp_ec','mvfx','mvfy','f','Spx','Spy')
