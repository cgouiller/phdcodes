alphalist=[0.1,0.5,1,1.5,2,2.5,3,4,5,8];
seuil=zeros(1,length(alphalist));
nt=5000;
old_nt=1;
taup=0;
dt=0.1/20;
...
for cpt=1:length(alphalist)
    ...
    while seuilmax-seuilmin>0.01 
        ...
        v=sqrt((mx(200:end)-mx(199:end-1)).^2+(my(200:end)-my(199:end-1)).^2);
        plot(v)
        if sum(v<1e-4)==0
            seuilmax=A;
        else
...
alphabar=alphalist*sig*sig/Dcamp;
Mc=seuil*1/(4*Dcamp*Dcamp);
x=1./besselk(0,alphabar);
x2=1./besselk(0,sqrt(alphabar));
    save('E:/Clément/SimuNum/Resultats/seuilnage.mat','seuil','alphabar','x','Mc','x2')
